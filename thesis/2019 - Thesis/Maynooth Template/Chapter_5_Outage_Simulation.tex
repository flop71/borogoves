...
Another consideration is the idea of event dependence. Typical off the shelf single-server queue models such as M/M/1 and G/G/1 assume that the inter-arrival and service times between events are independent \cite{kleinrock1975queuing}. However if some form of dependence is found between events how useful would a queuing model which assumes independence compare against that of a queuing model with dependence properties.
...
\section{Case study 5 - outage simulation}

\begin {table}
\begin{center}
\label{tab:chap5tab1}
\caption {Summary of dataset metrics} 
\begin{tabular}{| C{4cm} | C{9cm} |} \hline 
\textbf{Metric} & \textbf{Value}
\\ \hline Number of outage events & 331 
\\ \hline Data collection duration & 18 months (January 2015 to June 2016) 
\\ \hline Software components & Business Support System (BSS), Collaboration, E-mail, and Social  	
\\ \hline Number of Data Centres & 3 
\\ \hline Programming Language & Java 
\\ \hline Operating System & Linux 
\\ \hline Hardware & Intel based. (Mixture of physical and virtual systems)
\\ \hline Load Balancing & F5
\\ \hline
\end{tabular}
\end{center}
\end{table}
...
A number of points related to the dataset are summarised in Table \ref{tab:chap5tab1}. 

% added an extra paragraph to justify the metrics shown
It is important to affirm the Programming Language, OS, Hardware and Load Balancing solution used as part of the Enterprise solution. These metrics may be underlying factors in the root cause of the observed outages. Further investigation into the root cause of each outage event is beyond the scope of this case study. We discuss this matter in our study limitations subsection below.
...
\subsection{Outage event simulation framework}
...
Pearson \cite{fisher1915frequency} and Spearman's \cite{spearman1904proof} ranked coefficient use a single measurement to determine the relationship between two variables. The strength of the relationship is measured between 0 (no correlation) to 1 (high correlation). Additionally the coefficient can be positive or negative indicating the type of relationship. Pearson's test is typically used when dealing with a variables with a linear relationship while Spearman's test can be used where a relationship is monotonic (whether linear or not). 

Linear regression \cite{galton1890kinship} is a method to model the relationship between two variables where one variable is dependent and the other is an  independent variable. A hypothesis test is conducted and a p-value is computed. Depending on the size of the p-value the hypothesis of a relationship / no relationship can be accepted or rejected.
...
\subsection{Results - Inter-arrival time distribution}
...
\textbf{Distribution} & \textbf{AD Test Statistic} & \textbf{p-value}
\\ \hline Pareto & 0.53 & 0.72 
\\ \hline log-logistic & 1.93 & 0.10
\\ \hline log-normal & 3.79 & 0.01
\\ \hline gamma & 632.89 & 1.83e-06	
\\ \hline exponential & Infinity  & 1.83e-06
\\ \hline logistic & Infinity & 1.83e-06	
\\ \hline Weibull  & Infinity & 1.83e-06 
\\ \hline
\end{tabular}
\end{center}
\end{table}
...
\label{fig:chap5fig1}
\includegraphics[width=13cm, height=9cm]{graphs/obscured/inter_arrival_pareto.pdf} 
\end{center}
\end{figure}

Table \ref{tab:chap5tab2} shows a summary of the seven distributions fitted against the observed inter-arrival time data. Each distribution is listed along with its corresponding Anderson Darling test statistic and p-value. Fig \ref{fig:chap5fig1} shows four Goodness-of-fit plots for a fitted Pareto distribution: Density, Cumulative Distribution Function (CDF) , Probability (P-P) \cite{gibbons2011nonparametric}  and Quantile (Q-Q) \cite{wilk1968probability}.  

To answer the question of how are the inter-arrival times of Cloud outage events distributed, seven continuous distributions were fitted against the data set. To test the goodness of fit an Anderson-Darling (AD) Goodness of fit test was conducted against each probability distribution. 
...
\subsection{Results - Service time distribution}
...
\begin{tabular}{| C{4cm} | C{4cm} | C{4cm} |} \hline 
\textbf{Distribution} & \textbf{AD Test Statistic} & \textbf{p-value}
\\ \hline log-normal & 0.34 & 0.90
\\ \hline logl-ogistic & 0.74 & 0.53
\\ \hline Pareto & 1.60 & 0.15 
\\ \hline Weibull  & 6.82 & 4.00e-04 
\\ \hline gamma & 272.44 & 1.83e-06	
\\ \hline exponential & Infinity & 1.83e-06
\\ \hline logistic & Infinity  & 1.83e-06	
\\ \hline
\end{tabular}
...
\label{fig:chap5fig2}
\includegraphics[width=13cm, height=9cm]{graphs/obscured/service_lognormal.pdf} 
\end{center}
\end{figure}

Table \ref{tab:chap5tab3} shows a summary of the seven distributions fitted against the observed service time data. Each distribution is listed along with its corresponding Anderson Darling test statistic and p-value. Fig \ref{fig:chap5fig2} shows four Goodness-of-fit plots for a fitted lognormal distribution: Density, (CDF), (P-P)  and (Q-Q).  
...
\subsection{Results - Outage event modelling framework}

\begin {table}
\begin{center}
\caption {Summary of results from queue modelling experiments and observed overlapping outage events} 
\label{tab:chap5tab4}
\begin{tabular}{| C{5cm} | C{3.5cm} | C{3.5cm} |} \hline 
\textbf{Model Type} & \textbf {\% Busy} & \textbf {\% Free}
\\ \hline Observed Data & 7.9 &  92.1 
\\ \hline Simulation (G/G/1) & 5.7 &  94.3 
\\ \hline Simulation (M/M/1) & 3.0  &  97.0 
\\ \hline 
 \end{tabular}
\end{center}
\end{table}
...
\subsection{Results - Assessment for no association and linkage between overlapping outage events}

\begin {table}
\begin{flushleft}
\caption {Summary details of overlapping outages with analysis of component area, root cause and linkage assessment} 
\label{tab:chap5tab5}
\begin{tabular}{| C{2cm} | C{2cm} | C{2cm} | C{6cm} |} \hline 
\textbf{Outage} \# & \textbf{Component} & \textbf{Root Cause} & \textbf{Outage Details} 
...
\begin {table}
\begin{flushleft}
\caption {Summary details of overlapping outages with analysis of component area, root cause and linkage assessment (Continued)} 
\label{tab:chap5tab6}
\begin{tabular}{| C{2cm} | C{2cm} | C{2cm} | C{6cm} |} \hline 
\textbf{Outage} \# & \textbf{Component} & \textbf{Root Cause} & \textbf{Outage Details} 
\\ \hline 6 & E-mail & Configuration & A service on the E-mail system failed due to contention. A config change was made to remediate the initial contention. The config change caused additional contention further along the service stack. Assessment: Outages linked.
...
Prior to a detailed discussion of our results, we summarise the results along with each corresponding research question and relevance background literature. Table \ref{tab:chap5tab8} provides this summary

\subsection{Discussion - Inter-arrival time distribution}

% Research Question 1 answered here.
...
\subsection{Discussion - Service time distribution}

% Research Question 2 answered here.
...
\subsection{Discussion - Outage event modelling framework}

\begin {table}
\begin{center}
\caption {Sample output from our G/G/1 simulation} 
\label{tab:chap5tab9}
\begin{tabular}{| C{4cm} | C{4cm} | C{4cm} |} \hline 
\textbf{Duration (Minutes)} & \textbf{Queue Length}  & \textbf{Date \& Time}
\\ \hline 2  & 1 & 2017-01-01 00:02
\\ \hline 142 & 0 & 2017-01-01 02:22
\\ \hline 3744 & 1 & 2017-01-03 14:24 
\\ \hline 3761 & 0 & 2017-01-03 14:41
\\ \hline 5577 & 1 & 2017-01-04 20:57
\\ \hline 5644 & 0 & 2017-01-04 22:04
\\ \hline 11043 & 1 & 2017-01-08 16:03
\\ \hline 11048 & 0 & 2017-01-08 16:08
\\ \hline 14989 & 1 & 2017-01-11 09:49 
\\ \hline 15186 & 0 & 2017-01-11 13:06 
\\ \hline 19566 & 1 & 2017-01-14 14:06
\\ \hline 19605 & 0 & 2017-01-14 14:45
\\ \hline 22249 & 1 &  2017-01-16 10:49
\\ \hline 22278 & 2 & 2017-01-16 11:18
\\ \hline 22286 & 1 & 2017-01-16 11:26
\\ \hline
\end{tabular}
\end{center}
\end{table}
...
\subsection{Discussion - Correlation between inter-arrival and service times}
...
\subsection{Discussion - Assessment for no association and linkage between overlapping outage events}
...
Table Tables \ref{tab:chap5tab5} and \ref{tab:chap5tab6} provide a good insight into the nature of linked outage events we saw that in five of the seven linked outages that E-mail was a common component. Likewise we observed that network and configuration issues were the root causes in four of the seven outages, this may not be a coincidence. In one other case we saw that in a disaster recovery scenario, server node failover did not work as expected which caused a cascade failure due to high concurrency.

% Research Question 5 answered here. Added an additional sentence
Table \ref{tab:chap5tab7} highlights that overlapping outages are uncommon with approximately 8\% of all outages recorded over an eighteen month period overlapped. Additionally linked outages are rarer still with only approximately 2\% recorded over the same duration. However as demonstrated by the results of Fisher's test, there is overwhelming evidence to suggest that both events are associated. Therefore overlapping outage events are related. We can calculate when an overlapping event occurs there is a approximately a 25\% probability that these events are linked. Removal of these types of failures is key to the success of a business and will lead to increased customer satisfaction by increased up time. 
...
However the main application of this research is to DevOps and project planners within an SME or micro team. Both can leverage this framework to build an accurate resource planning model which can both identify skill and personnel gaps. Identification and remediation of these gaps will greatly benefit teams in the challenging area of Cloud outage resolution.
