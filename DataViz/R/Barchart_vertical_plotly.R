...
###############Read Data from csv ##################################
titanic.df <- read.csv(file="titanic.csv", header=TRUE, sep=",")
...
p.bar.v1 <- plot_ly(first.class.fare, x = ~first.class.fare$Pclass, y = ~mean(first.class.fare$Fare), type = 'bar')  %>%
...
# 1. Remove chart junk (Remove horizonal grid lines)
p.bar.v2 <- plot_ly(first.class.fare, x = ~first.class.fare$Pclass, y = ~mean(first.class.fare$Fare), type = 'bar',
              name = '1st Class')  %>%
  add_trace(second.class.fare, x = ~second.class.fare$Pclass, y = ~mean(second.class.fare$Fare), type = 'bar',
            name = '2nd Class')  %>%
  add_trace(third.class.fare, x = ~third.class.fare$Pclass, y = ~mean(third.class.fare$Fare), type = 'bar',
...
# 2. Using pre-attentative attributes lets adjust the colour of 2nd and 3rd class fare
p.bar.v3 <- plot_ly(first.class.fare, x = ~first.class.fare$Pclass, y = ~mean(first.class.fare$Fare), 
